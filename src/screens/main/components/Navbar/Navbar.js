        ...
        <Menu borderless style={{ borderRadius: 0 }} height='10px'>
            ...
            {loading ?
                ...
                <Menu.Menu position='right'>
                    ...
                    {/* ===== logged ===== */}
                    {/* admin items */}
                    <NavItemLoggedRole id='navbar-item-admin-1' to='/admin/calendar' iconName='calendar' text='main.navbar.calendar' isLoggedIn={isLoggedIn} role={ROLES.ADMIN} />
                    <NavItemLoggedRole id='navbar-item-admin-2' to='/admin/students' iconName='child' text='main.navbar.students' isLoggedIn={isLoggedIn} role={ROLES.ADMIN} />
                    <NavItemLoggedRole id='navbar-item-admin-3' to='/admin/teachers' iconName='female' text='main.navbar.teachers' isLoggedIn={isLoggedIn} role={ROLES.ADMIN} />
                    <NavItemLoggedRole id='navbar-item-admin-4' to='/admin/lessons' iconName='calendar alternate outline' text='main.navbar.lessons' isLoggedIn={isLoggedIn} role={ROLES.ADMIN} />
                    <NavItemLoggedRole id='navbar-item-admin-5' to='/admin/payments' iconName='money bill alternate' text='main.navbar.payments' isLoggedIn={isLoggedIn} role={ROLES.ADMIN} />                
                    {/* student items */}
                    ...
                    <NavItemLogged id='navbar-item-messaging' as={NavLink} to='/messaging' iconName='mail' text='main.navbar.messaging' isLoggedIn={isLoggedIn}/>
                    <NavItemLogged id='navbar-item-profile' as={NavLink} to='/profile' iconName='user' text='main.navbar.profile' isLoggedIn={isLoggedIn}/>
...
const NavItemLogged = (props) => {
    const {id, to, iconName, text, isLoggedIn} = props;
    if (!isLoggedIn) return null;
    return (
        <Menu.Item key={id} as={NavLink} to={to}>
            <Icon name={iconName}/>
            {I18n.t(text)}
        </Menu.Item>
    );
...
const NavItemLoggedRole = (props) => {
    const {id, to, iconName, text, isLoggedIn, role} = props;
